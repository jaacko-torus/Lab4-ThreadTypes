...
enum class ThreadType { Joinable, Detached };
...
int main(int argc, char **argv)
{
	...
	auto** threads = new std::thread*[totalThreadCount];
	...
	for(int i = totalThreadCount - 1; i >= 0; i--)
	{
		...
		perThreadData[i].threadType = i % 2 == 0 ? ThreadType::Joinable : ThreadType::Detached;
		threads[i] = new std::thread(i % 2 == 0 ? JoinableThreadEntrypoint : DetachedThreadEntrypoint, perThreadData);
	}
	...
	auto any_threads_are_joinable = true;
	while (any_threads_are_joinable)
	{
		auto any_threads_are_joinable_acc = true;

		for (int i = 0; i < totalThreadCount; i += 1)
		{
			if (perThreadData[i].threadType == ThreadType::Joinable)
			{
				if (threads[i]->joinable()) threads[i]->join();

				any_threads_are_joinable_acc &= threads[i]->joinable();
			}
		}

		any_threads_are_joinable = any_threads_are_joinable_acc;
	}
